...
const phaseEffects = {
    ...
    drawLightning(ctx) {
        ...
        if (Math.random() < 0.005) { // 0.5% de chance por frame
            ...
            ctx.lineWidth = 3;
            if (!(typeof polishSystem !== 'undefined' && polishSystem.effectiveQuality === 'low')) {
                ctx.shadowBlur = 20;
    ...
    // ===== PARTÍCULAS DE FOGO =====
    initFireParticles() {
        const q = (typeof polishSystem !== 'undefined') ? polishSystem.effectiveQuality : 'medium';
        const count = q === 'low' ? 10 : q === 'medium' ? 18 : 26;
    ...
    drawDarkFog(ctx) {
        ...
        const fogCount = (typeof polishSystem !== 'undefined' && polishSystem.effectiveQuality === 'low') ? 3 :
            (typeof polishSystem !== 'undefined' && polishSystem.effectiveQuality === 'medium') ? 6 : 10;
    ...
    initWarpLines() {
        const q = (typeof polishSystem !== 'undefined') ? polishSystem.effectiveQuality : 'medium';
        const count = q === 'low' ? 6 : q === 'medium' ? 10 : 15;
        for (let i = 0; i < count; i++) {
            this.warpLines.push({
                y: (i / count) * gameData.canvas.height,
    ...
    initChaosShapes() {
        const q = (typeof polishSystem !== 'undefined') ? polishSystem.effectiveQuality : 'medium';
        const count = q === 'low' ? 7 : q === 'medium' ? 14 : 22;
        for (let i = 0; i < count; i++) {
