...
const collisionSystem = {
    ...
    checkCircle(obj1, obj2) {
        ...
        // PERFORMANCE: comparar distância ao quadrado evita Math.sqrt() em
        // centenas/milhares de testes de colisão por segundo.
    ...
    // ✨ NOVO: Player vs Ovo de dragão (objetivo de resgate da fase)
    checkPlayerVsEgg() {
        for (let index = gameEntities.eggs.length - 1; index >= 0; index--) {
            const egg = gameEntities.eggs[index];
            if (this.checkCircle(dragon, egg)) {
                if (typeof eggSystem !== 'undefined') {
                    eggSystem.rescue(egg, index);
                } else {
    ...
    activateBomb() {
        ...
        const bombParticleCount = (typeof polishSystem !== 'undefined')
            ? (polishSystem.effectiveQuality === 'low' ? 24 : polishSystem.effectiveQuality === 'medium' ? 48 : 72)
            : 48;
    ...
    createImpactEffect(x, y, color) {
        const count = (typeof polishSystem !== 'undefined')
            ? (polishSystem.effectiveQuality === 'low' ? 3 : polishSystem.effectiveQuality === 'medium' ? 5 : 7)
            : 5;
    ...
    createCollectEffect(x, y, color) {
        const count = (typeof polishSystem !== 'undefined')
            ? (polishSystem.effectiveQuality === 'low' ? 4 : polishSystem.effectiveQuality === 'medium' ? 7 : 10)
            : 7;
        for (let i = 0; i < count; i++) {
