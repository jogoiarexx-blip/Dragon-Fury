...
const powerUpEffects = {
    ...
    update(powerups, playerX, playerY) {
        ...
        powerups.forEach(powerup => {
            ...
            const borderCount = quality === 'low' ? 1 : quality === 'medium' ? 2 : this.config.borderLayers;
            for (let i = 0; i < borderCount; i++) {
                powerup.borderRotations[i] = this.rotationPhase * (i + 1) * (i % 2 === 0 ? 1 : -1);
            }
            
            // Criar partículas apenas quando a qualidade comporta
            const spawnRate = quality === 'high' ? 0.25 : quality === 'medium' ? 0.10 : 0;
            if (particlesEnabled && Math.random() < spawnRate) {
    ...
    checkMagnetism(powerup, playerX, playerY) {
        ...
        const magnetSq = this.config.magnetDistance * this.config.magnetDistance;
        if (distanceSq < magnetSq) {
            ...
            const quality = (typeof polishSystem !== 'undefined') ? polishSystem.effectiveQuality : 'medium';
            if (quality !== 'low' && Math.random() < (quality === 'high' ? 0.08 : 0.03)) {
    ...
    draw(ctx, powerups) {
        const quality = (typeof polishSystem !== 'undefined') ? polishSystem.effectiveQuality : 'medium';
        if (quality !== 'low') this.drawMagneticEffects(ctx);
        for (let i = 0; i < powerups.length; i++) this.drawPowerUp(ctx, powerups[i]);
    ...
    onCollect(x, y, type, ctx) {
        ...
        // Explosão de partículas adaptativa
        const quality = (typeof polishSystem !== 'undefined') ? polishSystem.effectiveQuality : 'medium';
        const count = quality === 'low' ? 6 : quality === 'medium' ? 14 : 22;
        ...
        // Ondas de choque são caras; no modo baixo usamos só as partículas.
        if (quality !== 'low') this.createShockwave(x, y, colorScheme.glow, ctx);
    },
